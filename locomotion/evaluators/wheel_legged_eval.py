...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="wheel-legged-walking-v25000")
    ...
    env_cfg, obs_cfg, reward_cfg, command_cfg, curriculum_cfg, domain_rand_cfg, terrain_cfg, train_cfg = pickle.load(open(
        f"/home/huang/wheel_leg/wheel_legged_genesis_new/locomotion/logs/{args.exp_name}/cfgs.pkl", "rb"))
    ...
    pad = gamepad.control_gamepad(command_cfg,[1.0,2.0,3.14,0.0125])  # 初始化控制设备（手柄/键盘）
    with torch.no_grad():
        while True:
            # actions = policy(obs)
