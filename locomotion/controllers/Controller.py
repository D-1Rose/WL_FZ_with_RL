...
class BatchedLQRController:
    ...
    def __init__(self, num_envs: int, device: torch.device = None):
        ...
        self.command_pitch = BatchedPIDController(kp=20, ki=0.0, kd=1.2, num_envs=self.num_envs, device=self.device)
        self.command_velocity = BatchedPIDController(kp=20, ki=1.5, kd=0, num_envs=self.num_envs, device=self.device)
    ...
    def get_command_pitch(self):
        ...
        pitch_d_1 = self.pitch_d * ( torch.pi / 180 )
    ...
    def get_command_velocity(self):
        ...
        command = self.command_velocity.compute(error=theta_velocity, dt=0.002, stopping_mask=is_want_to_stop)
        return command
...
def main():
    ...
    import os
    import sys
    from FZ_RL_ENV import WheelLeggedEnv
    import sys
    ...
    # ========== 加载配置 ==========
    from FZ_RL_TRAIN import get_cfgs
    ...
    log_dir = os.path.join(current_dir, "logs", "controller_debug", timestamp)
    ...
    if args.use_keyboard:
        try:
            ...
            commands_scale = [0.5, 0.5, 1.5, 0.28]
    ...
    test_schedule = [
        (0.0,   0.1,  [-0.5,  0.0, 0.0, 0.28]),   # 0-10s:  向前走
        (0.1,   0.5,  [-0.5,  0.0, 0.0, 0.28]),   # 0-10s:  向前走
        (0.5,   0.8,  [-0.5,  0.0, 0.0, 0.28]),   # 0-10s:  向前走
        ...
        with torch.no_grad():
            while current_sim_time < max_test_time:
                ...
                if pad is not None:
                    ...
                    current_command = [0.0, 0.0, 0.0, 0.28] # 默认指令(防空)    
                
                    for (start, end, cmd) in test_schedule:
                        if start <= current_sim_time < end:
                            current_command = cmd
                            break # 找到对应时间段的指令后跳出查找
                    
                    # 4. 发送指令给环境 (注意 env.set_commands 接收 tensor 或 list)
                    # 这里的 0 代表第0个环境
                    ...
                    # 6. 打印状态 (每 0.5秒 打印一次，避免刷屏)
                    if current_step % 50 == 0:
                        print(f"Time: {current_sim_time:.2f}s | Cmd: {cmd} | Vel_x: {env.base_lin_vel[0,0]:.2f}")
                ...
                # (A) 跟踪性能
                writer.add_scalar('Tracking/Vel_X_Error', env.commands[0,0] - env.base_lin_vel[0,0], step_count)
                writer.add_scalar('Tracking/Command_Vel', env.commands[0,0], step_count)
                writer.add_scalar('Tracking/Vel_X', env.base_lin_vel[0,0], step_count)
                writer.add_scalar('Tracking/Height_Real', env.base_pos[0,2], step_count)
                
                # (B) 姿态与稳定性 (关键: 观察 Pitch 是否震荡)
                # 将弧度转换为度，更直观
                r = env.base_euler[env_id, 0]
                p = env.base_euler[env_id, 1]
                y = env.base_euler[env_id, 2]
                p_com = env.lqr_controller.pitch_com[0]
                p_vel = env.base_ang_vel[env_id, 1]
                writer.add_scalar('Attitude/Roll', r, step_count)
                writer.add_scalar('Attitude/Pitch', p, step_count)
                writer.add_scalar('Attitude/Yaw', y, step_count)
                writer.add_scalar('Attitude/Pitch_Vel', p_vel, step_count)
                writer.add_scalar('Attitude/Pitch_Com', p_com, step_count)
                
                # (D) 模糊参数 (观察参数变化)
                # 注意：如果 actions 是 0，这里看到的应该是默认值
                fuzzy = env.lqr_controller.current_fuzzy_params[env_id]
                writer.add_scalar('Params/Gamma_d_g', fuzzy[0], step_count)
                writer.add_scalar('Params/Eth', fuzzy[1], step_count)
                writer.add_scalar('Params/A', fuzzy[2] if len(fuzzy)>2 else 0, step_count)
