...
class BatchedLQRController:
    ...
    def __init__(self, num_envs: int, device: torch.device = None):
        ...
        self.pitch_d = 0 * torch.ones(num_envs, device=self.device) # 特殊变量保留
        ...
        # PID 控制器结构初始化 (参数稍后填充)
        self.command_yaw_dot = BatchedPIDController(kp=1, ki=0.0, kd=0, num_envs=self.num_envs, device=self.device)
...
# [必须添加] 显式初始化这三个参数，防止 reset 时 KeyError

        self.defaults['gamma_d_g'] = 75.0 * math.pi / 180.0
        self.defaults['eth'] = 0.05
        self.defaults['a'] = 0.05

        self.params['gamma_d_g'] = torch.full((self.num_envs,), 75.0 * math.pi / 180.0, device=self.device)
        self.params['eth'] = torch.full((self.num_envs,), 0.05, device=self.device)
        self.params['a'] = torch.full((self.num_envs,), 0.05, device=self.device)
    ...
    def get_command_pitch(self):
        ...
        pitch_d   = self.pitch_com*0 + pitch_ref_from_vel
    ...
    def get_command_yaw_dot(self):
        ...
        command = self.command_yaw_dot.compute(error=theta_yaw_dot, dt=0.002)
        return command
    ...
    def ST_SIT2_FLC_FM(self, error_pass: torch.Tensor):
        ...
        # [修改点] 不再用 self.fuzzy_xxx，改用 self.params['xxx']
        gamma_d_g = self.params['gamma_d_g']
        eth       = self.params['eth']
        ld_max    = 0.9
        ld_min    = 0.3
        a         = self.params['a']
...
class BatchedVMC:
    ...
    def update_params(self, new_params_dict):
        """接收来自 RL 环境的腿部残差指令"""
        self.params.update(new_params_dict)
    ...
    def vmc(self):
        ...
        # ================== VMC 力计算 (完美融合) ==================
        # 目标位置项 (k * Error) 融入了 RL 的主动缩腿意图 (z_d + delta_z)
        # 前馈补偿项 (+/- z_d_l) 保留了抗倾覆能力
        F_l = torch.stack([
...
def main():
    ...
    # ========== 加载配置 ==========
    from locomotion.FZ_PID_train import get_cfgs
    ...
    terrain_cfg["eval"] = "terrain_wave"
    terrain_cfg["horizontal_scale"] = 0.02
    ...
    # ========== [新增] 初始化 TensorBoard ==========
    # 创建一个独立的日志目录，方便对比不同参数的实验
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_dir = os.path.join(current_dir, "logs", "FZ_PID_c_debug", timestamp)
    ...
    if args.use_keyboard:
        try:
            ...
            keyboard = KeyboardCommander(v_step=0.1, yaw_step_deg=1.0)

            sys.path.append("/home/huang/wheel_leg_paper/wheel_legged_genesis/locomotion")
            from utils import gamepad
    ...
    # ========== 设置测试命令 ==========
    if pad is None:
        # [修改] 不再设置死板的固定命令，而是准备开始自动化测试
        print("🤖 未检测到键盘 (--use_keyboard)，将启用 [自动化分段测试]")
        print("   - 0~10s: 前进")
        print("   - 10~20s: 停止")
        print("   - 20~30s: 后退")
        print("   - 30~40s: 停止\n")
        ...
        with torch.no_grad():
            while True:
                ...
                if pad is not None:
                    ...
                    # [新增逻辑] 自动化分段测试
                    # 计算当前时间 (假设仿真步长 dt=0.01)
                    current_time = step_count * 0.002
                    
                    # 默认指令: [vx, vy, w_z, h] -> 原地站立
                    cmd = [0.0, 0.0, 0.0, 0.28]
                    vel = -2
                    # 编写剧本
                    if 0 <= current_time < 2.0:
                        cmd = [0.0, 0.0, 0.0, 0.28]   # 前进
                    elif 2.0 <= current_time < 10.0:
                        cmd = [vel, 0.0, 0.0, 0.28]   # 前进
                    elif 10.0 <= current_time < 20.0:
                        cmd = [vel, 0.0, 0.0, 0.28]   # 停
                    # elif 20.0 <= current_time < 30.0:
                    #     cmd = [vel, 0.0, 0.0, 0.28]  # 后退
                    # elif 30.0 <= current_time < 50.0:
                    #     cmd = [vel, 0.0, 0.0, 0.28]   # 停
                    ...
                    # 发送指令 (给第0个环境)
                    env.set_commands(0, cmd)
                # ----- [新增] 4. TensorBoard 数据记录 -----
                # 我们只记录第 0 个环境的数据，避免混乱
                step_count += 1
                env_id = 0
...
# (A) 跟踪性能 (Tracking)
                # 观察上坡提速和下坡刹车的核心指标
                writer.add_scalar('Tracking/Vel_X_Error', env.commands[env_id, 0] - env.base_lin_vel[env_id, 0], step_count)
                writer.add_scalar('Tracking/Command_Vel', env.commands[env_id, 0], step_count)
                writer.add_scalar('Tracking/Vel_X_Real', env.base_lin_vel[env_id, 0], step_count)
                writer.add_scalar('Tracking/Height_Real', env.base_pos[env_id, 2], step_count)
                
                # (B) 姿态与稳定性 (单位：度)
                # 增加 Deg 后缀，提醒自己在观察度数
                r_deg = env.base_euler[env_id, 0] * 180 / math.pi
                p_deg = env.base_euler[env_id, 1] * 180 / math.pi
                y_deg = env.base_euler[env_id, 2] * 180 / math.pi
                p_com_deg = env.lqr_controller.pitch_com[env_id] * 180 / math.pi
                p_vel = env.base_ang_vel[env_id, 1] # 俯仰角速度

                writer.add_scalar('Attitude/Roll_Deg', r_deg, step_count)
                writer.add_scalar('Attitude/Pitch_Deg', p_deg, step_count)
                writer.add_scalar('Attitude/Yaw_Deg', y_deg, step_count)
                writer.add_scalar('Attitude/Pitch_Vel', p_vel, step_count)
                writer.add_scalar('Attitude/Pitch_CoM_Ref_Deg', p_com_deg, step_count)
                
                # (C) RL 残差动作 (观察大脑对小脑的实时干预)

                writer.add_scalar('Params/gamma_d_g', env.lqr_controller.params['gamma_d_g'][env_id], step_count)
                
                # (D) 关节状态 (Dofs)
                # 记录左轮速和关节力矩，观察爬坡时的动力输出
                writer.add_scalar('Joints/Left_Wheel_Vel', env.dof_vel[env_id, 4], step_count)
                writer.add_scalar('Joints/Left_Knee_Pos', env.dof_pos[env_id, 1], step_count)
                writer.add_scalar('Joints/Total_Force_Norm', torch.norm(env.dof_force[env_id]), step_count)

                # (E) 奖励观测 (仅测试模式下参考)
                writer.add_scalar('Reward/Step_Total', env.rew_buf[env_id], step_count)

                # # ----- 5. 定期控制台打印 -----
                # if step_count % print_interval == 0:
                #     # 保持原来的打印逻辑，这里省略以节省篇幅...
                #     print(f"Step: {step_count} | Pitch: {p:.2f}° | Vx: {env.base_lin_vel[env_id, 0]:.2f} | Roll: {r:.2f}° ")
        ...
        print("\n\n" + "="*60)
        print("⏹️  测试中断")
        print(f"📂 记得查看 TensorBoard: {log_dir}")
        print("="*60)
    finally:
        writer.close() # 别忘了关闭 writer
