import math
from transforms3d import euler
import numpy as np
import genesis as gs
from genesis.utils.geom import quat_to_xyz, transform_by_quat, inv_quat, transform_quat_by_quat  # type: ignore
from genesis.engine.solvers.rigid.rigid_solver_decomp import RigidSolver
import torch
import time

import torch

class BatchedPIDController:
    def __init__(self, kp, ki, kd, num_envs, device=None, dtype=torch.float32):
        self.kp = torch.as_tensor(kp, dtype=dtype, device=device)
        self.ki = torch.as_tensor(ki, dtype=dtype, device=device)
        self.kd = torch.as_tensor(kd, dtype=dtype, device=device)

        self.device = device if device is not None else torch.device("cpu")

        # (num_envs,)
        self.prev_error = torch.zeros(num_envs, dtype=dtype, device=self.device)
        self.integral   = torch.zeros(num_envs, dtype=dtype, device=self.device)

    def compute(self, error, dt, stopping_mask=None):
        # 1. 默认状态下保持较弱的衰减，维持爬坡动力
        leak_rate = 0.995 
        
        # 2. [核心修改] 当处于静止掩码时，使用极强的衰减 (0.5)
        # 这样积分不会瞬间消失(导致剧烈跳动)，但在 2-3 帧内会迅速归零
        if stopping_mask is not None:
            # 在需要定住的环境，让积分迅速缩水
            leak_rate_tensor = torch.where(stopping_mask, 0.5, 0.995)
            self.integral *= leak_rate_tensor
        else:
            self.integral *= leak_rate

        self.integral += error * dt
        # 增大限幅，给爬坡留足力矩空间
        self.integral = torch.clamp(self.integral, -20.0, 20.0) 

        derivative = (error - self.prev_error) / dt
        output = self.kp * error + self.ki * self.integral + self.kd * derivative
        self.prev_error = error  
        return output



class BatchedLQRController:
    """
    将你原来的 LQR_Controller 初始化部分向量化（batched，num_envs）
    使用示例：
        ctrl = BatchedLQRController(num_envs, robot, device=device)
    假定 robot 在并行模式下的接口返回形状 (num_envs, ...) 的 torch tensors
    """
    def __init__(self, num_envs: int, device: torch.device = None):
        self.num_envs = int(num_envs)
        self.device = torch.device('cpu') if device is None else device
        # self.robot = robot
        self.dt = 0.01
        self.joint_name_to_idx = None
        # base_quat 保留原意，但并行化为 tensor (num_envs, 4)
        self.base_quat = torch.zeros((self.num_envs, 4), device=self.device, dtype=gs.tc_float)
        self.base_init_quat = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device)
        # ========== 模糊参数默认值（直接向量化，不再是 scalar） ==========
        d = lambda x: torch.full((num_envs,), float(x), device=device)

        self.fuzzy_gamma_d_g_default = d(75 * math.pi / 180)
        self.fuzzy_eth_default       = d(0.06)
        self.fuzzy_ld_max_default    = d(0.9)
        self.fuzzy_ld_min_default    = d(0.3)
        self.fuzzy_a_default         = d(0.1)

        self.pitch_kp_default = d(0.5) 
        self.pitch_kd_default = d(0.015)
        self.vel_kp_default   = d(10.0)#可以控制速度响应的大小
        self.vel_ki_default   = d(6) #可以控制速度响应的大小还有倾倒范围（正相关）,可以提供爬坡的力矩，但是在指令为零的时候剩余的能量会导致机器人“迟滞”运动
        # self.pitch_d_default  = d(0)

        # ========== delta 范围，同样向量化 ==========
        self.gamma_d_g_delta = d(50 * math.pi / 180)
        self.eth_delta       = d(0.06)  #以下的增量都不要超出其默认值，如果超出则在env中动作更新的时候产生负值导致错误
        self.ld_max_delta    = d(0.9)
        self.ld_min_delta    = d(0.3)
        self.a_delta         = d(0.1)

        # self.pitch_d_delta = d(25)

        # # Pitch Kp 范围: [0.05, 0.5]
        # self.pitch_kp_min     = d(0.1) 
        # self.pitch_kp_max     = d(3)  
        # # Velocity Kp 范围: [1.0, 10.0]
        # self.vel_kp_min       = d(1)  
        # self.vel_kp_max       = d(5.0)

        # ========== 当前可调模糊参数（默认与 default 相同） ==========
        self.fuzzy_gamma_d_g = self.fuzzy_gamma_d_g_default.clone()
        self.fuzzy_eth       = self.fuzzy_eth_default.clone()
        self.fuzzy_ld_max    = self.fuzzy_ld_max_default.clone()
        self.fuzzy_ld_min    = self.fuzzy_ld_min_default.clone()
        self.fuzzy_a         = self.fuzzy_a_default.clone()

        self.pitch_kp         = self.pitch_kp_default.clone()
        self.vel_kp           = self.vel_kp_default.clone()
        self.pitch_kd         = self.pitch_kd_default.clone()
        self.vel_ki           = self.vel_ki_default.clone()
        # self.pitch_d          = self.pitch_d_default.clone()

        # ================== 状态量（全部张量化，保证每个 env 独立） ==================
        z = torch.zeros((self.num_envs,), device=self.device, dtype=torch.float32)
        # position / velocity / efforts for relevant joints
        self.left_wheel_velocity  = z.clone()
        self.left_wheel_position  = z.clone()
        self.right_wheel_velocity = z.clone()
        self.right_wheel_position = z.clone()
        self.left_hip_position    = z.clone()
        self.right_hip_position   = z.clone()
        self.left_knee_position   = z.clone()
        self.right_knee_position  = z.clone()

        self.left_hip_effort  = z.clone()
        self.left_knee_effort = z.clone()
        self.left_wheel_effort = z.clone()
        self.right_hip_effort = z.clone()
        self.right_knee_effort = z.clone()
        self.right_wheel_effort = z.clone()

        # imu / com related
        self.roll       = z.clone()
        self.pitch      = z.clone()
        self.pitch_com  = z.clone()
        self.n          = z.clone()
        self.pitch_dot  = z.clone()
        self.left_knee_velocity  = z.clone()
        self.right_knee_velocity = z.clone()
        self.left_hip_velocity   = z.clone()
        self.right_hip_velocity  = z.clone()

        # dynamics intermediate vars (batched)
        self.theta_r1 = z.clone(); self.theta_r2 = z.clone(); self.theta_r3 = z.clone()
        self.theta_rb = z.clone()
        self.theta_r0_dot = z.clone(); self.theta_r1_dot = z.clone(); self.theta_r2_dot = z.clone()
        self.theta_r3_dot = z.clone(); self.theta_rb_dot = z.clone()

        self.theta_l1 = z.clone(); self.theta_l2 = z.clone(); self.theta_l3 = z.clone()
        self.theta_lb = z.clone()
        self.theta_l0_dot = z.clone(); self.theta_l1_dot = z.clone(); self.theta_l2_dot = z.clone()
        self.theta_l3_dot = z.clone(); self.theta_lb_dot = z.clone()

        # torques / control outputs
        self.tao_rh = z.clone(); self.tao_rk = z.clone(); self.tao_rw = z.clone()
        self.tao_lh = z.clone(); self.tao_lk = z.clone(); self.tao_lw = z.clone()

        # other scalar states
        self.error_pitch = z.clone()
        self.error_pitch_dot = z.clone()
        self.desired_state = z.clone()
        self.prev_error = z.clone()
        self.integral = z.clone()
        self.robot_x_velocity = z.clone()
        self.integral_yaw = z.clone()
        self.yaw_dot = z.clone()
        self.yaw = z.clone()
        self.robot_x_position = z.clone()
        self.velocity_d = z.clone()
        self.roll_dot = z.clone()

        # init command states
        self.pitch_d    = 0 * torch.ones(num_envs, device=self.device)#特殊

        self.velocity_d = torch.zeros(num_envs, device=self.device) * torch.ones(num_envs, device=self.device)
        self.yaw_d_dot      = 0 * torch.ones(num_envs, device=self.device)

        # ================== 批量 PID 控制器（每个 env 各有一组 PID 状态） ==================
        self.command_yaw_dot = BatchedPIDController(kp=2, ki=0.0, kd=0, num_envs=self.num_envs, device=self.device)
        self.command_pitch = BatchedPIDController(kp=self.pitch_kp, ki=0, kd=self.pitch_kd, num_envs=self.num_envs, device=self.device)
        self.command_velocity = BatchedPIDController(kp=self.vel_kp, ki=self.vel_ki, kd=0, num_envs=self.num_envs, device=self.device)



    def set_joint_mapping(self, joint_name_to_idx):
        """设置关节索引映射（由环境传入）"""
        self.joint_name_to_idx = joint_name_to_idx



    def set_fuzzy_params(self, gamma_d_g, eth, ld_max, ld_min, a , pitch_kp ,pitch_kd ,vel_kp ,vel_ki , 
                        #  pitch_target
                         ): 

        def strict(x, name):
            if not torch.is_tensor(x):
                x = torch.as_tensor(x, device=self.device, dtype=torch.float32)
            x = x.flatten()
            if x.shape[0] != self.num_envs:
                raise ValueError(f"{name} must be shape ({self.num_envs},), got {tuple(x.shape)}")
            return x

        gamma_d_g = strict(gamma_d_g, "gamma_d_g")
        eth       = strict(eth,       "eth")
        ld_max    = strict(ld_max,    "ld_max")
        ld_min    = strict(ld_min,    "ld_min")
        a         = strict(a,         "a")
        pitch_kp  = strict(pitch_kp,  "pitch_kp")  # 新增验证
        vel_kp    = strict(vel_kp,    "vel_kp")    # 新增验证

        self.fuzzy_gamma_d_g = torch.clamp(
            gamma_d_g,
            self.fuzzy_gamma_d_g_default - self.gamma_d_g_delta,
            self.fuzzy_gamma_d_g_default + self.gamma_d_g_delta
        )
        self.fuzzy_eth = torch.clamp(
            eth,
            self.fuzzy_eth_default - self.eth_delta,
            self.fuzzy_eth_default + self.eth_delta
        )
        self.fuzzy_ld_max = torch.clamp(
            ld_max,
            self.fuzzy_ld_max_default - self.ld_max_delta,
            self.fuzzy_ld_max_default + self.ld_max_delta
        )
        self.fuzzy_ld_min = torch.clamp(
            ld_min,
            self.fuzzy_ld_min_default - self.ld_min_delta,
            self.fuzzy_ld_min_default + self.ld_min_delta
        )
        self.fuzzy_a = torch.clamp(
            a,
            self.fuzzy_a_default - self.a_delta,
            self.fuzzy_a_default + self.a_delta
        )

        # self.pitch_d = torch.clamp(
        #     pitch_target,
        #     self.pitch_d_default - self.pitch_d_delta,
        #     self.pitch_d_default + self.pitch_d_delta
        # )

        #  =======增量式的都像以上那样裁切处理=======

        self.pitch_kp = pitch_kp
        self.pitch_kd = pitch_kd
        self.vel_kp = vel_kp
        self.vel_ki = vel_ki


        self.current_fuzzy_params = torch.stack([
            self.fuzzy_gamma_d_g,
            self.fuzzy_eth,
            self.fuzzy_ld_max,
            self.fuzzy_ld_min,
            self.fuzzy_a,
            self.pitch_kp,
            self.pitch_kd,
            self.vel_kp,
            self.vel_ki,
            # self.pitch_d

        ], dim=1)  # shape: [num_envs, 5]



    def set_commands(self, velocity_d=None, yaw_d_dot=None):
        """支持外界指令设置，必须为 shape (num_envs,) 的张量"""
        if velocity_d is not None:
            self.velocity_d = velocity_d
        if yaw_d_dot is not None:
            self.yaw_d_dot = yaw_d_dot


    
    def set_state(self, dof_pos, dof_vel, base_vel, base_euler, base_ang ):
        """
        从zhu环境接收状态（替代 update_joint_states 和 update_imu_data）
        
        Args:
            dof_pos: [num_envs, num_dofs] 所有关节位置
            dof_vel: [num_envs, num_dofs] 所有关节速度
            quat: [num_envs, 4] 机体四元数
            ang: [num_envs, 3] 机体角速度
        """
        # 确保数据在正确设备上
        dof_pos = dof_pos.to(self.device)
        dof_vel = dof_vel.to(self.device)
        base_vel = base_vel.to(self.device)
        base_euler = base_euler.to(self.device)

        # quat = quat.to(self.device)
        ang = base_ang.to(self.device)
        
        # 提取需要的关节
        self.position_lw = dof_pos[:, self.joint_name_to_idx["left_wheel"]]
        self.position_rw = dof_pos[:, self.joint_name_to_idx["right_wheel"]]
        self.position_lh = dof_pos[:, self.joint_name_to_idx["left_hip"]]
        self.position_rh = dof_pos[:, self.joint_name_to_idx["right_hip"]]
        self.position_lk = dof_pos[:, self.joint_name_to_idx["left_knee"]]
        self.position_rk = dof_pos[:, self.joint_name_to_idx["right_knee"]]
        
        self.velocity_lw = dof_vel[:, self.joint_name_to_idx["left_wheel"]]
        self.velocity_rw = dof_vel[:, self.joint_name_to_idx["right_wheel"]]
        self.velocity_lh = dof_vel[:, self.joint_name_to_idx["left_hip"]]
        self.velocity_rh = dof_vel[:, self.joint_name_to_idx["right_hip"]]
        self.velocity_lk = dof_vel[:, self.joint_name_to_idx["left_knee"]]
        self.velocity_rk = dof_vel[:, self.joint_name_to_idx["right_knee"]]
        
        # 更新 IMU 数据
        self.roll_dot = ang[:, 0]* torch.pi/180
        self.pitch_dot = ang[:, 1]
        self.yaw_dot = ang[:, 2]
        
        # self.base_quat = quat
        # self.inv_base_init_quat = inv_quat(self.base_init_quat)
        
        # aligned_quat = transform_quat_by_quat(
        #     torch.ones_like(self.base_quat) * self.inv_base_init_quat,
        #     self.base_quat
        # )
        # rpy = quat_to_xyz(aligned_quat)
        euler = base_euler
        
        self.roll = euler[:, 0]* torch.pi/180
        self.pitch = euler[:, 1]
        self.yaw = euler[:, 2]

        # ====== 动力学模型参数计算(保留你原始公式) ======
        # pitch, pitch_dot 在 update_imu_data() 更新
        pitch     = self.pitch      # shape: [num_envs]
        pitch_dot = self.pitch_dot  # shape: [num_envs]
        # right leg
        self.theta_r1 = self.position_rw
        self.theta_r2 = -(95/180*math.pi - self.position_rk)
        self.theta_r3 = self.position_rh - 20/180*math.pi
        self.theta_rb = -self.theta_r2 - self.theta_r3 - math.pi/2 + pitch

        self.theta_r1_dot = self.velocity_rw
        self.theta_r2_dot = self.velocity_rk
        self.theta_r3_dot = self.velocity_rh
        self.theta_rb_dot = -self.theta_r2_dot - self.theta_r3_dot + pitch_dot

        # left leg
        self.theta_l1 = -self.position_lw
        self.theta_l2 = -(95/180*math.pi + self.position_lk)
        self.theta_l3 = -self.position_lh - 20/180*math.pi
        self.theta_lb = -self.theta_l2 - self.theta_l3 - math.pi/2 + pitch

        self.theta_l1_dot = -self.velocity_lw
        self.theta_l2_dot = -self.velocity_lk
        self.theta_l3_dot = -self.velocity_lh
        self.theta_lb_dot = -self.theta_l2_dot - self.theta_l3_dot + pitch_dot

        # 机器人线速度 vx，取世界坐标系 X 方向
        self.robot_x_velocity = base_vel[:, 0]   # 保持tensor

        # 用轮子差分推算位置 (可选，不建议用于真实正解)
        self.robot_x_position = (self.position_lw - self.position_rw) * 0.1 / 2.0



    def wrap_to_pi(self, angles):
    # 这是标准的 wrap to pi 逻辑
        return (angles + torch.pi) % (2 * torch.pi) - torch.pi

    def get_command_pitch(self):
        # pitch_d 已由外部 set_commands 设置
        L, M, self.pitch_com, roll_com, Jz, Jy = self.com(
            self.right_hip_position,
            self.right_knee_position,
            self.left_hip_position,
            self.left_knee_position,
            self.roll

        )

        pitch_d_1 = self.pitch_d
        # pitch_com = self.pitch_com * (180 / torch.pi)
        pitch_d   = self.pitch_com - pitch_d_1

        theta_pitch = pitch_d - self.pitch

        theta_pitch_pass = self.ST_SIT2_FLC_FM(theta_pitch)

        command = self.command_pitch.compute(error=theta_pitch_pass, dt=0.01)
        # print(f"euler:{self.base_euler}")
        # print(f"self.pitch_d:{self.pitch_d}")
        # print(f"self.pitch:{self.pitch}")
        # print(f"self,pitch_com:{self.pitch_com}")

        return command  # shape: (num_envs,)

    def get_command_velocity(self):
        theta_velocity = self.robot_x_velocity - self.velocity_d
        
        # [逻辑重构]
        # 只要指令不为零 (abs > 0.01)，哪怕我在爬坡慢如蜗牛，也不许进入 stopping 状态
        is_want_to_stop = (torch.abs(self.velocity_d) < 0.01) 
        
        # 只有真正想停，才把 mask 传进去强制衰减积分
        command = self.command_velocity.compute(
            error=theta_velocity, 
            dt=0.01, 
            stopping_mask=is_want_to_stop
        )
        return command

    def get_command_yaw_dot(self):
        theta_yaw_dot = self.yaw_d_dot - self.yaw_dot
        command = self.command_yaw_dot.compute(error=theta_yaw_dot, dt=0.01)
        return command

    def com(self, xita_r1, xita_r2, xita_l1, xita_l2, roll):
        # 保持原先角度偏置逻辑
        xita_r1 = xita_r1 - 20 * torch.pi / 180
        xita_r2 = xita_r2 - 95 * torch.pi / 180
        xita_l1 = -xita_l1 - 20 * torch.pi / 180
        xita_l2 = -xita_l2 - 95 * torch.pi / 180

        # 常量部分保持不变（转 torch 并放到与输入相同的 device）
        device = xita_r1.device
        lr1 = 150e-3
        lr2 = 250e-3
        ll1 = lr1
        ll2 = lr2
        dlr = 355e-3

        dr1 = torch.tensor([0.020031, 0.0032153, 0.0036019], device=device)
        dr2 = torch.tensor([0.072881, 0.034135, 0.019602], device=device)
        dl1 = torch.tensor([0.019888, 0.0032269, -0.0036874], device=device)
        dl2 = torch.tensor([0.073104, 0.034096, -0.019634], device=device)
        db  = torch.tensor([0.01131, -0.0013618, -0.00028518], device=device)

        mr1 = 0.51378
        mr2 = 0.27188
        ml1 = 0.51183
        ml2 = 0.27057
        mb  = 2.9637

        # 批量 trigonometric 运算
        cr1 = torch.cos(xita_r1)
        sr1 = torch.sin(xita_r1)
        cl1 = torch.cos(xita_l1)
        sl1 = torch.sin(xita_l1)

        cr1r2 = torch.cos(xita_r1 + xita_r2)
        sr1r2 = torch.sin(xita_r1 + xita_r2)
        cl1l2 = torch.cos(xita_l1 + xita_l2)
        sl1l2 = torch.sin(xita_l1 + xita_l2)

        def _expand_z(val, ref): 
            return torch.full_like(ref, val)

        Xr1 = torch.stack([
            cr1 * dr1[0] - sr1 * dr1[1],
            sr1 * dr1[0] + cr1 * dr1[1],
            _expand_z(dr1[2] + dlr / 2, xita_r1)
        ], dim=-1)

        Xr2 = torch.stack([
            cr1r2 * dr2[0] - sr1r2 * dr2[1] + lr1 * cr1,
            sr1r2 * dr2[0] + cr1r2 * dr2[1] + lr1 * sr1,
            _expand_z(dr2[2] + dlr / 2, xita_r1)
        ], dim=-1)

        Xl1 = torch.stack([
            cl1 * dl1[0] - sl1 * dl1[1],
            sl1 * dl1[0] + cl1 * dl1[1],
            _expand_z(dl1[2] - dlr / 2, xita_r1)
        ], dim=-1)

        Xl2 = torch.stack([
            cl1l2 * dl2[0] - sl1l2 * dl2[1] + ll1 * cl1,
            sl1l2 * dl2[0] + cl1l2 * dl2[1] + ll1 * sl1,
            _expand_z(dl2[2] - dlr / 2, xita_r1)
        ], dim=-1)

        Xb = torch.stack([
            _expand_z(db[0], xita_r1),
            _expand_z(db[1], xita_r1),
            _expand_z(db[2], xita_r1)
        ], dim=-1)

        # 质心计算
        Xbody = (mr1 * Xr1 + mr2 * Xr2 + ml1 * Xl1 + ml2 * Xl2 + mb * Xb) / (mr1 + mr2 + ml1 + ml2 + mb)

        T_80_3 = torch.stack([
            (lr2 * torch.cos(xita_r1 + xita_r2) + lr2 * torch.cos(xita_l1 + xita_l2)
            + lr1 * torch.cos(xita_l1) + lr1 * torch.cos(xita_r1)) / 2,
            (lr2 * torch.sin(xita_l1 + xita_l2) + lr2 * torch.sin(xita_r1 + xita_r2)
            + lr1 * torch.sin(xita_l1) + lr1 * torch.sin(xita_r1)) / 2,
            torch.zeros_like(xita_r1)
        ], dim=-1)

        X8_re = Xbody - T_80_3

        # 机体 roll 变换
        c = torch.cos(-roll)
        s = torch.sin(-roll)
        Rx_roll = torch.stack([
            torch.stack([torch.ones_like(c), torch.zeros_like(c), torch.zeros_like(c)], dim=-1),
            torch.stack([torch.zeros_like(c), c, -s], dim=-1),
            torch.stack([torch.zeros_like(c), s,  c], dim=-1)
        ], dim=-2)

        X8_re = torch.matmul(Rx_roll, X8_re.unsqueeze(-1)).squeeze(-1)

        pitch_com = torch.atan(X8_re[:, 0] / X8_re[:, 1])
        roll_com  = torch.atan(X8_re[:, 2] / X8_re[:, 1])

        L = torch.sqrt(X8_re[:, 0]**2 + X8_re[:, 1]**2)

        # 转动惯量等效质量计算
        def dist(a, b):
            return torch.sqrt(torch.sum((a - b) ** 2, dim=-1))

        rr1 = dist(Xr1, T_80_3)
        rr2 = dist(Xr2, T_80_3)
        rl1 = dist(Xl1, T_80_3)
        rl2 = dist(Xl2, T_80_3)
        rb  = dist(Xb,  T_80_3)

        m = (mr2*rr2**2 + ml2*rl2**2 + mr1*rr1**2 + ml1*rl1**2 + mb*rb**2) / (L**2)
        J_z = m * L**2

        # roll 惯量
        def dist_r(a, b):
            return torch.sqrt((a[:, 0] - b[:, 0])**2 + (a[:, 2] - b[:, 2])**2)

        rr1_r = dist_r(Xr1, T_80_3)
        rr2_r = dist_r(Xr2, T_80_3)
        rl1_r = dist_r(Xl1, T_80_3)
        rl2_r = dist_r(Xl2, T_80_3)
        rb_r  = dist_r(Xb,  T_80_3)

        J_y = mr2*rr2_r**2 + ml2*rl2_r**2 + mr1*rr1_r**2 + ml1*rl1_r**2 + mb*rb_r**2

        return L, m, pitch_com, roll_com, J_z, J_y


    def ST_SIT2_FLC_FM(self, error_pass: torch.Tensor):
        """
        批处理 SIT2-FLC 模糊调节控制器
        参数:
            error_pass: shape = [num_envs], rad误差
        返回:
            phi: shape = [num_envs], rad控制输出
        """
        # ---------- 误差归一化（保持你原来的逻辑） ----------
        error = error_pass / math.pi * 180 / 15   # shape [N]

        # ---------- 取模糊参数（均为 shape [N] 张量） ----------
        gamma_d_g = self.fuzzy_gamma_d_g
        eth       = self.fuzzy_eth
        ld_max    = self.fuzzy_ld_max
        ld_min    = self.fuzzy_ld_min
        a         = self.fuzzy_a

        # ---------- ld_g计算 ----------
        # np.cos → torch.cos
        ld_g = (torch.cos(gamma_d_g) - eth) / torch.cos(gamma_d_g)

        B0_g = torch.zeros_like(ld_g)
        B1_g = ld_g * torch.sin(gamma_d_g)
        B2_g = torch.ones_like(ld_g)

        C0_g = torch.zeros_like(ld_g)
        C1_g = ld_g * torch.cos(gamma_d_g)
        C2_g = torch.ones_like(ld_g)

        K0_g = (B1_g - B0_g) / (C1_g - C0_g)
        K1_g = (B2_g - B1_g) / (C2_g - C1_g)
        N0_g = (B0_g * C1_g - B1_g * C0_g) / (C1_g - C0_g)
        N1_g = (B1_g * C2_g - B2_g * C1_g) / (C2_g - C1_g)

        # ---------- gamma_g 分段计算 (if/elif 改为tensor where) ----------
        cond1 = (error <= -C2_g)
        cond2 = (error > -C2_g) & (error < -C1_g)
        cond3 = (error >= -C1_g) & (error < C1_g)
        cond4 = (error >= C1_g) & (error < C2_g)
        cond5 = (error >= C2_g)

        gamma_g = torch.zeros_like(error)

        gamma_g = torch.where(cond1, B2_g * 70 + 10, gamma_g)
        gamma_g = torch.where(cond2, (K1_g * torch.abs(error) + N1_g) * 70 + 10, gamma_g)
        gamma_g = torch.where(cond3, (K0_g * torch.abs(error)) * 70 + 10, gamma_g)
        gamma_g = torch.where(cond4, (K1_g * error + N1_g) * 70 + 10, gamma_g)
        gamma_g = torch.where(cond5, B2_g * 70 + 10, gamma_g)

        # ---------- ld_e ----------
        gamma_g_1 = gamma_g * math.pi / 180
        ld_e = torch.abs(error) / torch.cos(gamma_g_1)
        ld_e = torch.clamp(ld_e, ld_min, ld_max)
        ld = ld_e

        B0 = torch.zeros_like(ld)
        B1 = ld * torch.sin(gamma_g_1)
        B2 = torch.ones_like(ld)

        C0 = torch.zeros_like(ld)
        C1 = ld * torch.cos(gamma_g_1)
        C2 = torch.ones_like(ld)

        m0 = a
        m1 = 1 - a
        m2 = a

        # ---------- K0, K1, N0, N1 ----------
        K0 = 0.5 * ((B1 - B0 * m0) / (C1 * m0 - C0 + torch.abs(error) * (-m0 + 1)) +
                    (B0 - B1 * m1) / (C0 * m1 - C1 + torch.abs(error) * (-m1 + 1)))

        K1 = 0.5 * ((B2 - B1 * m1) / (C2 * m1 - C1 + torch.abs(error) * (-m1 + 1)) +
                    (B1 - B2 * m2) / (C1 * m2 - C2 + torch.abs(error) * (-m2 + 1)))

        N0 = 0.5 * ((B1 * C0 - B0 * C1 * m0) / (-C1 * m0 + C0 + torch.abs(error) * (m0 - 1)) +
                    (B0 * C1 - B1 * C0 * m1) / (-C0 * m1 + C1 + torch.abs(error) * (m1 - 1)))

        N1 = 0.5 * ((B2 * C1 - B1 * C2 * m1) / (-C2 * m1 + C1 + torch.abs(error) * (m1 - 1)) +
                    (B1 * C2 - B2 * C1 * m2) / (-C1 * m2 + C2 + torch.abs(error) * (m2 - 1)))

        # ---------- φ 分段 ----------
        cond1 = (error <= -C2)
        cond2 = (error > -C2) & (error < -C1)
        cond3 = (error >= -C1) & (error < C1)
        cond4 = (error >= C1) & (error < C2)
        cond5 = (error >= C2)

        phi = torch.zeros_like(error)

        phi = torch.where(cond1, -B2*15/180*math.pi + (error + C2), phi)
        phi = torch.where(cond2, (K1 * error - N1)*15/180*math.pi, phi)
        phi = torch.where(cond3, (K0 * error)*15/180*math.pi, phi)
        phi = torch.where(cond4, (K1 * error + N1)*15/180*math.pi, phi)
        phi = torch.where(cond5, B2*15/180*math.pi + (error - C2), phi)

        return phi  # shape [num_envs]
    
    def balance(self,):
        a = self.get_command_pitch()
        b = self.get_command_velocity()
        c = self.get_command_yaw_dot()
        left_command = -0.5*a+0.5*b-c
        right_command = 0.5*a-0.5*b-c
        left_command  = left_command.to(dtype=torch.float32)
        right_command = right_command.to(dtype=torch.float32)
        wheel_forces = torch.stack([left_command, right_command], dim=1)

        # self.robot.control_dofs_force(
        #     wheel_forces,
        #     [
        #         self.joint_name_to_idx["left_wheel"],
        #         self.joint_name_to_idx["right_wheel"]
        #     ]
        # )
        return wheel_forces


class BatchedVMC:
    def __init__(self, num_envs: int, device: torch.device = None):
        self.num_envs = int(num_envs)
        self.device = torch.device('cpu') if device is None else device
        # self.robot = robot
        self.dt = 0.01
        self.base_init_quat = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device)
        self.joint_name_to_idx = None
        # ========= 向量化创建工具 =========
        def z():
            return torch.zeros((self.num_envs,), device=self.device, dtype=torch.float32)

        # ================== 关节位置（全并行张量化） ==================
        self.position_lh = z()  # 左髋
        self.position_lk = z()  # 左膝
        self.position_rh = z()  # 右髋
        self.position_rk = z()  # 右膝
        self.position_lw = z()  # 左轮
        self.position_rw = z()  # 右轮

        # ================== 关节速度 ==================
        self.velocity_lh = z()
        self.velocity_lk = z()
        self.velocity_rh = z()
        self.velocity_rk = z()
        self.velocity_lw = z()
        self.velocity_rw = z()

        # ================== IMU 姿态 ==================
        self.roll     = z()   # shape = (num_envs,)
        self.roll_dot = z()   # shape = (num_envs,)

        # 四元数 shape = (num_envs, 4)
        self.base_quat = torch.zeros((self.num_envs, 4), device=self.device, dtype=torch.float32)

        # ================== 关节索引映射（与 Genesis motor_dofs 对应） ==================
        # self.joint_names = [
        #     "L1_joint", "L2_joint", "R1_joint", "R2_joint", "L3_joint", "R3_joint"
        # ]
        
        # self.joint_name_to_idx = {
        #     "left_wheel":  11, # motor_dofs[4]
        #     "right_wheel": 10, # motor_dofs[5]
        #     "left_hip":    7,  # motor_dofs[0]
        #     "left_knee":   9,  # motor_dofs[1]
        #     "right_hip":   6,  # motor_dofs[2]
        #     "right_knee":  8   # motor_dofs[3]
        # }
    def set_joint_mapping(self, joint_name_to_idx):
        """设置关节索引映射"""
        self.joint_name_to_idx = joint_name_to_idx        


    # def update_joint_states(self):
    #     # motor dofs index (保持原始风格)
    #     self.motor_dofs = [self.robot.get_joint(name).dof_idx_local for name in self.joint_names]

    #     left_wheel_index  = self.motor_dofs[4]
    #     right_wheel_index = self.motor_dofs[5]
    #     left_hip_index    = self.motor_dofs[0]
    #     right_hip_index   = self.motor_dofs[2]
    #     left_knee_index   = self.motor_dofs[1]
    #     right_knee_index  = self.motor_dofs[3]

    #     # 全部一次提取，避免重复 API 调用（更高性能）
    #     pos = self.robot.get_dofs_position()   # shape: [num_envs, num_dofs]
    #     vel = self.robot.get_dofs_velocity()
    #     eff = self.robot.get_dofs_force()
    #     base_vel = self.robot.get_vel()        # 机器人线速度

    #     # === 关节角度（不再 float(...)，保持为 tensor） ===
    #     self.position_lw = pos[:, left_wheel_index]
    #     self.position_rw = pos[:, right_wheel_index]
    #     self.position_lh = pos[:, left_hip_index]
    #     self.position_rh = pos[:, right_hip_index]
    #     self.position_lk = pos[:, left_knee_index]
    #     self.position_rk = pos[:, right_knee_index]

    #     # === 关节速度 ===
    #     self.velocity_lw = vel[:, left_wheel_index]
    #     self.velocity_rw = vel[:, right_wheel_index]
    #     self.velocity_lh = vel[:, left_hip_index]
    #     self.velocity_rh = vel[:, right_hip_index]
    #     self.velocity_lk = vel[:, left_knee_index]
    #     self.velocity_rk = vel[:, right_knee_index]

    #     # === 力矩 Effort ===
    #     self.left_wheel_effort  = eff[:, left_wheel_index]
    #     self.right_wheel_effort = eff[:, right_wheel_index]
    #     self.left_hip_effort    = eff[:, left_hip_index]
    #     self.right_hip_effort   = eff[:, right_hip_index]
    #     self.left_knee_effort   = eff[:, left_knee_index]
    #     self.right_knee_effort  = eff[:, right_knee_index]

    # def update_imu_data(self):

    #     ang = self.robot.get_ang()  # (num_envs, 3)
    #     # 保持变量名称不变
    #     self.roll_dot  = ang[:, 0]   # w_x

    #     # quat = self.robot.get_quat()   # (num_envs, 4) assumed [w,x,y,z]
    #     # w, x, y, z = quat[:, 0], quat[:, 1], quat[:, 2], quat[:, 3]

    #     # # 等价原来的 np 调用：rpy = euler.quat2euler([w,x,y,z])
    #     # rpy = quat_to_xyz(quat)  # 如果其数学含义一致则可用，否则必须自己写欧拉变换
    #     # self.roll = rpy[:, 0]
    #     # self.pitch = rpy[:, 1]
    #     # self.yaw = rpy[:, 2]



    #     # Genesis quat format = [w, x, y, z]
    #     quat = self.robot.get_quat()  # (num_envs, 4), torch tensor
    #     self.base_quat[:] = quat      # 写入已有缓存

    #     self.inv_base_init_quat = inv_quat(self.base_init_quat)#[1.0, 0.0, 0.0, 0.0]

    #     # 将世界坐标系姿态旋转到初始 IMU 对齐坐标系
    #     aligned_quat = transform_quat_by_quat(
    #         torch.ones_like(self.base_quat) * self.inv_base_init_quat,  # (num_envs, 4)
    #         self.base_quat                                               # (num_envs, 4)
    #     )
    #     # 转为 XYZ 欧拉角 (roll, pitch, yaw)
    #     # 输出 shape: (num_envs, 3)
    #     rpy = quat_to_xyz(aligned_quat)
    #     # ================== 保持你原有的 self.roll 等 API 接口 ==================
    #     self.roll  = rpy[:, 0]
    #     self.pitch = rpy[:, 1]
    #     self.yaw   = rpy[:, 2]
    #     # print(f"aligned_quat: {aligned_quat}")
    #     # print(f"quat: {quat}")
    #     # print(f"rpy: {rpy}")
    #     # print(f"self.roll: {self.roll}")
    #     # print(f"self.pitch: {self.pitch}")
    #     # print(f"self.yaw: {self.yaw}")


    def set_state(self, dof_pos, dof_vel, base_vel, base_euler, ang):
        """从环境接收状态"""
        dof_pos = dof_pos.to(self.device)
        dof_vel = dof_vel.to(self.device)
        base_vel = base_vel.to(self.device)

        base_euler = base_euler.to(self.device)

        # quat = quat.to(self.device)
        ang = ang.to(self.device)
        
        # 提取关节状态
        self.position_lw = dof_pos[:, self.joint_name_to_idx["left_wheel"]]
        self.position_rw = dof_pos[:, self.joint_name_to_idx["right_wheel"]]
        self.position_lh = dof_pos[:, self.joint_name_to_idx["left_hip"]]
        self.position_rh = dof_pos[:, self.joint_name_to_idx["right_hip"]]
        self.position_lk = dof_pos[:, self.joint_name_to_idx["left_knee"]]
        self.position_rk = dof_pos[:, self.joint_name_to_idx["right_knee"]]
        
        self.velocity_lw = dof_vel[:, self.joint_name_to_idx["left_wheel"]]
        self.velocity_rw = dof_vel[:, self.joint_name_to_idx["right_wheel"]]
        self.velocity_lh = dof_vel[:, self.joint_name_to_idx["left_hip"]]
        self.velocity_rh = dof_vel[:, self.joint_name_to_idx["right_hip"]]
        self.velocity_lk = dof_vel[:, self.joint_name_to_idx["left_knee"]]
        self.velocity_rk = dof_vel[:, self.joint_name_to_idx["right_knee"]]
        
        # 更新 IMU
        self.roll_dot = ang[:, 0]* torch.pi/180
        # self.base_quat = quat
        
        # self.inv_base_init_quat = inv_quat(self.base_init_quat)
        # aligned_quat = transform_quat_by_quat(
        #     torch.ones_like(self.base_quat) * self.inv_base_init_quat,
        #     self.base_quat
        # )
        # rpy = quat_to_xyz(aligned_quat)
        # self.roll = rpy[:, 0]

        euler = base_euler
        self.roll = euler[:, 0]* torch.pi/180
        self.pitch = euler[:, 1]
        self.yaw = euler[:, 2]
    
    def vmc(self):
        # ================== 原始标量参数，保持不变 ==================
        k = 400  # 120
        d = 40
        theta_10 = 70 * math.pi / 180
        theta_20 = 95 * math.pi / 180
        l1 = 0.15
        l2 = 0.25
        x_d = 0.0353
        z_d = 0.2779
        x_dot_d = 0.0
        z_dot_d = 0.0

        # ================== 张量化 roll 力补偿 ==================
        z_d_l =( 2500 * self.roll + 100 * self.roll_dot ) # (num_envs,)

        # ================== 连杆位置 ==================
        # 注意：sin/cos 用 torch 版本，不改变表达式结构
        x_l = l1 * torch.sin(theta_10 - self.position_lh) \
            - l2 * torch.sin((theta_20 + self.position_lk) - (theta_10 - self.position_lh))

        z_l = l1 * torch.cos(theta_10 - self.position_lh) \
            + l2 * torch.cos((theta_20 + self.position_lk) - (theta_10 - self.position_lh))

        x_r = l1 * torch.sin(theta_10 + self.position_rh) \
            - l2 * torch.sin((theta_20 - self.position_rk) - (theta_10 + self.position_rh))

        z_r = l1 * torch.cos(theta_10 + self.position_rh) \
            + l2 * torch.cos((theta_20 - self.position_rk) - (theta_10 + self.position_rh))

        # ================== 雅可比矩阵（矢量化） ==================
        # shape: (num_envs, 2, 2)
        jcobi_l = torch.stack([
            torch.stack([
                -l1 * torch.cos(theta_10 - self.position_lh)
                - l2 * torch.cos((theta_20 + self.position_lk) - (theta_10 - self.position_lh)),

                -l2 * torch.cos((theta_20 + self.position_lk) - (theta_10 - self.position_lh))
            ], dim=1),
            torch.stack([
                l1 * torch.sin(theta_10 - self.position_lh)
                - l2 * torch.sin((theta_20 + self.position_lk) - (theta_10 - self.position_lh)),

                -l2 * torch.sin((theta_20 + self.position_lk) - (theta_10 - self.position_lh))
            ], dim=1)
        ], dim=1)

        jcobi_r = torch.stack([
            torch.stack([
                l1 * torch.cos(theta_10 + self.position_rh)
                + l2 * torch.cos((theta_20 - self.position_rk) - (theta_10 + self.position_rh)),

                l2 * torch.cos((theta_20 - self.position_rk) - (theta_10 + self.position_rh))
            ], dim=1),
            torch.stack([
                -l1 * torch.sin(theta_10 + self.position_rh)
                + l2 * torch.sin((theta_20 - self.position_rk) - (theta_10 + self.position_rh)),

                l2 * torch.sin((theta_20 - self.position_rk) - (theta_10 + self.position_rh))
            ], dim=1)
        ], dim=1)

        # ================== 关节角速度 ==================
        theta_dot_l = torch.stack([self.velocity_lh, self.velocity_lk], dim=1).unsqueeze(-1)
        theta_dot_r = torch.stack([self.velocity_rh, self.velocity_rk], dim=1).unsqueeze(-1)

        # ================== 足端速度 ==================
        # p_dot shape -> (num_envs, 2)
        p_dot_l = torch.bmm(jcobi_l, theta_dot_l).squeeze(-1)
        p_dot_r = torch.bmm(jcobi_r, theta_dot_r).squeeze(-1)

        # ================== VMC 力 ==================
        F_l = torch.stack([
            k*(x_d - x_l) + d*(x_dot_d - p_dot_l[:, 0]),
            k*(z_d - z_l) + d*(z_dot_d - p_dot_l[:, 1]) - z_d_l
        ], dim=1)

        F_r = torch.stack([
            k*(x_d - x_r) + d*(x_dot_d - p_dot_r[:, 0]),
            k*(z_d - z_r) + d*(z_dot_d - p_dot_r[:, 1]) + z_d_l
        ], dim=1)

        # ================== 关节力矩 ==================
        # τ = J^T F
        tao_l = torch.bmm(jcobi_l.transpose(1, 2), F_l.unsqueeze(-1)).squeeze(-1)
        tao_r = torch.bmm(jcobi_r.transpose(1, 2), F_r.unsqueeze(-1)).squeeze(-1)

        ## ================== 发送控制到 Genesis ==================
        # dofs = [
        #     self.joint_name_to_idx["left_hip"],
        #     self.joint_name_to_idx["left_knee"],
        #     self.joint_name_to_idx["right_hip"],
        #     self.joint_name_to_idx["right_knee"],
        # ]

        force = torch.stack([
            tao_l[:, 0],
            tao_l[:, 1],
            tao_r[:, 0],
            tao_r[:, 1],
        ], dim=1)  # shape (num_envs, 4)

        # #limit force range:need to set.........

        # self.robot.control_dofs_force(force, dofs)

        return force

# def main():
#     gs.init(backend=gs.gpu)
#     import torch
#     import cv2
#     import numpy as np

#     from wheel_legged_train_our import get_cfgs  
#     env_cfg, _, _, _, _, _, terrain_cfg = get_cfgs()

#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     dt = 0.002  

#     # ========== 创建并行仿真场景 ==========
#     scene = gs.Scene(
#         sim_options=gs.options.SimOptions(dt=dt, substeps=2),
#         viewer_options=gs.options.ViewerOptions(
#             max_FPS=int(0.5 / dt),
#             camera_pos=(2.0, 0.0, 2.5),
#             camera_lookat=(0.0, 0.0, 0.5),
#             camera_fov=40,
#         ),
#         vis_options=gs.options.VisOptions(n_rendered_envs=1),
#         rigid_options=gs.options.RigidOptions(
#             dt=dt,
#             constraint_solver=gs.constraint_solver.Newton,
#             enable_collision=True,
#             enable_joint_limit=True,
#             batch_dofs_info=True,
#         ),
#         show_viewer=True,
#     )

#     # ========== 地面与地形 ==========
#     scene.add_entity(gs.morphs.URDF(file="assets/terrain/plane/plane.urdf", fixed=True))

#     if terrain_cfg["terrain"]:
#         height_field = cv2.imread(
#             "assets/terrain/png/" + terrain_cfg["train"] + ".png",
#             cv2.IMREAD_GRAYSCALE
#         )
#         scene.add_entity(gs.morphs.Terrain(
#             height_field=height_field,
#             horizontal_scale=terrain_cfg["horizontal_scale"],
#             vertical_scale=terrain_cfg["vertical_scale"],
#         ))

#     # ========== 添加并行机器人 ==========
#     base_init_quat = np.array(env_cfg["base_init_quat"], dtype=np.float32)

#     robot = scene.add_entity(
#         gs.morphs.URDF(
#             file="/home/zhjkoi/wheel_leg_paper/wheel_legged_genesis/wheel_leg/src/description/urdf/wheel_leg.urdf",
#             pos=(0, 0, 0.40),
#             quat=base_init_quat,
#         ),
#     )
#     n_envs = 3
#     scene.build(n_envs=n_envs,env_spacing=(1, 1))

#     # ========== 初始化控制器 ==========
#     vmc = BatchedVMC(n_envs, robot , device=device)
#     lqr = BatchedLQRController(n_envs, robot , device=device)



#     # =================== pid test ====================
#     # pid = BatchedPIDController(kp=1.2, ki=0.8, kd=0.15, num_envs=3, device="cuda")

#     # # 对三个环境施加三个不同误差
#     # error = torch.tensor([-1.0, 0.0, 1.0], device="cuda")
#     # dt = 0.02

#     # u = pid.compute(error, dt)
#     # print(u)     # tensor([..., ..., ...], device='cuda:0')


#     # ========== 主循环 ==========
#     while True:
#         # ----- VMC 控制 -----
#         vmc.update_joint_states()
#         vmc.update_imu_data()
#         vmc.vmc()   # output hip/knee torques (batched)

#         # ----- LQR 控制 -----
#         lqr.update_joint_states()
#         lqr.update_imu_data()
#         lqr.balance()  # batch torque shape [3, 2] for 2 wheels

#         # ----- Step -----
#         scene.step()
# if __name__ == "__main__":
#     main()

# num_envs_fz_control.py 的 main() 函数

def main():
    import argparse
    import os
    import sys
    from wl_fz_env import WheelLeggedEnv
    import sys
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)
    print(current_dir)
    print(parent_dir)
    sys.path.append(parent_dir)
    from utils import gamepad
    parser = argparse.ArgumentParser(description="测试模糊控制器")
    parser.add_argument("--num_envs", type=int, default=1, help="并行环境数量")
    parser.add_argument("--use_keyboard", action="store_true", help="使用键盘控制")
    args = parser.parse_args()
    
    print("\n" + "="*60)
    print("    模糊控制器测试 - Fuzzy Controller Test")
    print("="*60 + "\n")
    
    # ========== 初始化 Genesis ==========
    gs.init(backend=gs.cuda, logging_level="warning")
    
    # ========== 加载配置 ==========
    from wl_fz_train import get_cfgs
    env_cfg, obs_cfg, reward_cfg, command_cfg, curriculum_cfg, domain_rand_cfg, terrain_cfg = get_cfgs()
    
    # 测试配置调整
    terrain_cfg["terrain"] = True
    terrain_cfg["train"] = "agent_train_gym"
    
    print("📋 配置信息:")
    print(f"  - 并行环境数: {args.num_envs}")
    print(f"  - 地形: {terrain_cfg['train']}")
    print(f"  - 动作维度: {env_cfg.get('num_actions', 7)}")
    print(f"  - 控制频率: 100 Hz")
    print()
    
    # ========== 创建环境 ==========
    print("🔧 初始化环境...")
    env = WheelLeggedEnv(
        num_envs=args.num_envs,
        env_cfg=env_cfg,
        obs_cfg=obs_cfg,
        reward_cfg=reward_cfg,
        command_cfg=command_cfg,
        curriculum_cfg=curriculum_cfg,
        domain_rand_cfg=domain_rand_cfg,
        terrain_cfg=terrain_cfg,
        robot_morphs="urdf",
        show_viewer=True,
        train_mode=False,
        device="cuda:0"
    )
    print("✅ 环境初始化成功!")
    print(f"✅ LQR 控制器已就绪 (device: {env.lqr_controller.device})")
    print(f"✅ VMC 控制器已就绪 (device: {env.vmc_controller.device})")
    print()
    
    # ========== 初始化键盘控制 ==========
    pad = None
    if args.use_keyboard:
        try:
            sys.path.append("/home/zhjkoi/wheel_leg_paper/wheel_legged_genesis/locomotion")
            from utils import gamepad
            
            commands_scale = [2.0, 1.0, 1.5, 0.28]
            pad = gamepad.control_gamepad(command_cfg, commands_scale)
            print("🎮 键盘控制已启用")
            print("   - 使用方向键控制机器人")
            print("   - 按 'R' 重置环境\n")
        except Exception as e:
            print(f"⚠️  键盘控制初始化失败: {e}")
            print("   将使用固定命令测试\n")
    else:
        print("🤖 使用固定命令测试")
        print("   提示: 添加 --use_keyboard 启用键盘控制\n")
    
    # ========== 重置环境 ==========
    obs, _ = env.reset()
    print(f"📊 初始观测形状: {obs.shape}")
    print(f"   期望: [{args.num_envs}, {env.num_obs}]\n")
    
    # ========== 设置测试命令 ==========
    if pad is None:
        test_commands = torch.tensor(
            [[0.5, 0.0, 0.0, 0.28]] * args.num_envs,
            device="cuda:0"
        )
        env.commands[:] = test_commands
        print("🎯 设置测试命令: [vel_x=0.5, vel_y=0.0, ang_z=0.0, height=0.28]\n")
    
    # ========== 主测试循环 ==========
    print("="*60)
    print("🚀 开始测试循环 (按 Ctrl+C 退出)")
    print("="*60 + "\n")
    
    step_count = 0
    print_interval = 100
    
    try:
        with torch.no_grad():
            while True:
                # ----- 1. 生成测试动作 -----
                actions = torch.randn(args.num_envs, env.num_actions, device="cuda:0") * 0.0
                
                # ----- 2. 环境步进 -----
                obs, _, rews, dones, infos = env.step(actions)
                
                # ----- 3. 键盘控制 -----
                if pad is not None:
                    commands, reset_flag = pad.get_commands()
                    env.set_commands([0], commands)
                    
                    if reset_flag:
                        print("\n🔄 手动重置环境...\n")
                        env.reset()
                        step_count = 0
                        continue
                
                # ----- 4. 定期打印状态 -----
                if step_count % print_interval == 0:
                    env_id = 0  # 只打印第一个环境
                    
                    print(f"\n{'='*60}")
                    print(f"Step: {step_count} | Time: {step_count * 0.01:.2f}s")
                    print(f"{'='*60}")
                    
                    # 命令
                    print(f"\n[Command]")
                    print(f"  vel_x: {env.commands[env_id, 0]:.3f}")
                    print(f"  vel_y: {env.commands[env_id, 1]:.3f}")
                    print(f"  ang_z: {env.commands[env_id, 2]:.3f}")
                    print(f"  height: {env.commands[env_id, 3]:.3f}")
                    
                    # 基座状态
                    print(f"\n[Base State]")
                    print(f"  lin_vel: [{env.base_lin_vel[env_id, 0]:.3f}, "
                          f"{env.base_lin_vel[env_id, 1]:.3f}, "
                          f"{env.base_lin_vel[env_id, 2]:.3f}]")
                    print(f"  ang_vel: [{env.base_ang_vel[env_id, 0]:.3f}, "
                          f"{env.base_ang_vel[env_id, 1]:.3f}, "
                          f"{env.base_ang_vel[env_id, 2]:.3f}]")
                    print(f"  euler (r,p,y): [{env.base_euler[env_id, 0]:.3f}, "
                          f"{env.base_euler[env_id, 1]:.3f}, "
                          f"{env.base_euler[env_id, 2]:.3f}]")
                    print(f"  position: [{env.base_pos[env_id, 0]:.3f}, "
                          f"{env.base_pos[env_id, 1]:.3f}, "
                          f"{env.base_pos[env_id, 2]:.3f}]")
                    
                    # 模糊参数
                    print(f"\n[Fuzzy Params]")
                    fuzzy = env.lqr_controller.current_fuzzy_params[env_id]
                    print(f"  gamma_d_g: {fuzzy[0]:.4f}")
                    print(f"  eth:       {fuzzy[1]:.4f}")
                    print(f"  ld_max:    {fuzzy[2]:.4f}")
                    print(f"  ld_min:    {fuzzy[3]:.4f}")
                    print(f"  a:         {fuzzy[4]:.4f}")
                    
                    # 关节状态
                    print(f"\n[Joint State]")
                    print(f"  dof_pos: [{env.dof_pos[env_id, 0]:.3f}, "
                          f"{env.dof_pos[env_id, 1]:.3f}, "
                          f"{env.dof_pos[env_id, 2]:.3f}, "
                          f"{env.dof_pos[env_id, 3]:.3f}, "
                          f"{env.dof_pos[env_id, 4]:.3f}, "
                          f"{env.dof_pos[env_id, 5]:.3f}]")
                    print(f"  dof_vel: [{env.dof_vel[env_id, 0]:.3f}, "
                          f"{env.dof_vel[env_id, 1]:.3f}, "
                          f"{env.dof_vel[env_id, 2]:.3f}, "
                          f"{env.dof_vel[env_id, 3]:.3f}, "
                          f"{env.dof_vel[env_id, 4]:.3f}, "
                          f"{env.dof_vel[env_id, 5]:.3f}]")
                    
                    # 控制器状态
                    print(f"\n[Controller]")
                    print(f"  LQR pitch: {env.lqr_controller.pitch[env_id]:.3f}")
                    print(f"  LQR pitch_dot: {env.lqr_controller.pitch_dot[env_id]:.3f}")
                    print(f"  VMC roll: {env.vmc_controller.roll[env_id]:.3f}")
                    print(f"  VMC roll_dot: {env.vmc_controller.roll_dot[env_id]:.3f}")
                    



                    # print(f"\n[Forces]")
                    # print(f"  lw: {env.step.leg_torques[env_id, 0]:.3f}, ")
                    # print(f"  rw: {env.lqr_controller.balance[env_id, 1]:.3f}, ")
                    # 异常检测
                    warnings = []
                    if abs(env.base_euler[env_id, 1]) > 0.5:
                        warnings.append(f"Pitch过大: {env.base_euler[env_id, 1]:.3f}")
                    if abs(env.base_euler[env_id, 0]) > 0.5:
                        warnings.append(f"Roll过大: {env.base_euler[env_id, 0]:.3f}")
                    if env.base_pos[env_id, 2] < 0.15:
                        warnings.append(f"高度过低: {env.base_pos[env_id, 2]:.3f}")
                    
                    if warnings:
                        print(f"\n⚠️  警告:")
                        for w in warnings:
                            print(f"  - {w}")
                
                step_count += 1
                
    except KeyboardInterrupt:
        print("\n\n" + "="*60)
        print("⏹️  测试中断")
        print("="*60)
        print(f"总步数: {step_count}")
        print(f"仿真时间: {step_count * 0.01:.2f}s")
        print("✅ 控制器测试完成!\n")


if __name__ == "__main__":
    main()
