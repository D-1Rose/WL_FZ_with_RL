...
class WheelLeggedEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, curriculum_cfg, 
                 domain_rand_cfg, terrain_cfg, robot_morphs="urdf", show_viewer=True, device="cuda:0", train_mode=True):
        ...
        self.num_actions = env_cfg["num_actions"]  # 6
        ...
        match robot_morphs:
            case "urdf":
                self.robot = self.scene.add_entity(
                    gs.morphs.URDF(
                        file="/home/huang/wheel_leg/wheel_legged_genesis_new/assets/urdf/wheel_leg/urdf/wheel_leg2.urdf",
                        pos = base_init_pos,
    ...
    def _reward_similar_legged(self):
        # 两侧腿相似 适合使用轮子行走 抑制劈岔，要和_reward_knee_height结合使用,简化控制：对称性约束减少了动作空间维度，加速学习过程
        legged_error = torch.sum(torch.square(self.dof_pos[:,0:2] - self.dof_pos[:,2:4]), dim=1)  # 这里确保你的dof_motor的顺序正确，对应关节,(左腿-右腿)关节尽可能小
        return torch.exp(-legged_error / self.reward_cfg["tracking_similar_legged_sigma"])
        # return legged_error

    def _reward_knee_height(self):
        # 关节处于某个范围惩罚，避免总跪着
        left_knee_idx = torch.abs(self.left_knee_pos[:, 2]) < 0.07
        right_knee_idx = torch.abs(self.right_knee_pos[:, 2]) < 0.07
    ...
    def _reward_pitch(self):
        # 保持水平奖励 使用pitch角度
        pitch_error = torch.square(self.base_euler[:, 1]-0)
